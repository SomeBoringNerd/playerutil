    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args)
    {
        ...
        }else
        {
            ...
            try {
                ...
                {
                    ResultSet set2 = Playerutil.prepareStatement("SELECT LastSeen FROM " + (Playerutil.Debug ? "player_data_debug" : "player_data") + " WHERE UUID='" + player.getUniqueId().toString().replace("-", "") + "';").executeQuery();
                    set2.next();
                    sender.sendMessage("[§52b2fr§r] : here's the date §4" + args[0] +  "§r was seen for the last time : §4" + set2.getDate("LastSeen").toString());
                }
